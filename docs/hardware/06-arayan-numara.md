...
- **Güvenlik:** Webhook ucu herkese açık (`@Public`) ama **HMAC-SHA256 imza + tazelik (timestamp) doğrulaması** ister; imza/sürede taze değilse **fail-closed** reddedilir (imzasız çağrı asla kabul edilmez). `mock` sağlayıcı yalnızca dev/staging'de çalışır, **production'da 403** ile kapalıdır. Uç, dakikada 30 istekle throttle'lıdır.
