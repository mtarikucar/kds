...
- Buluta yalnız **giden HTTPS** (443, istek/yanıt; kalıcı soket yok) ile bağlanır; **20 sn'de bir** `POST /v1/bridges/heartbeat` gönderir. (Kalıcı WSS push / `/ws/bridge` **planlanan**.)
- `device_commands` kuyruğundan cihaz komutlarını `GET /v1/bridges/:id/commands/next` ile yoklayarak çeker, yerel sürücülere dağıtır.
...
| Ağ davranışı | Yalnız **giden** bağlantı: buluta HTTPS (443, istek/yanıt) + LAN yazıcıya raw-TCP 9100. **WAN portu açmaz, yerel sunucu/broker dinlemez.** |
...
1. **Açılış:** Köprü açık ve LAN'a bağlıdır. Boot'ta bearer token'ı çözer (bugün `HUMMY_BRIDGE_TOKEN` ortam değişkeninden; kalıcı OS keyring saklama **planlanan** durumdadır — bkz. Bölüm 4.2/10), buluta giden **HTTPS** ile **heartbeat** başlar (kalıcı WSS push planlanan). Admin panelde köprü **`online`** görünür.
...
5. Köprü bearer'ı saklar, buluta giden **HTTPS** ile **20 sn'de bir** `POST /v1/bridges/heartbeat` gönderir ve iş için `GET /v1/bridges/:id/commands/next` yoklar; sonuçları `POST /v1/devices/commands/:id/ack` ile bildirir (kalıcı WSS push / `/ws/bridge` **planlanan**). 60 sn sessizlikte bulut köprüyü `offline` yapar.
...
- Köprünün **dışarı (443/HTTPS) çıkışı** açık olmalı. Köprü **WAN tarafına port açmaz**; yalnız giden bağlantı kurar (buluta HTTPS + LAN yazıcıya raw-TCP 9100).
- Kurumsal/otel ağında güvenlik duvarı 443 giden HTTPS trafiğini engellememeli.
...
| Boot sonrası köprü **online olmuyor** | Bearer ortamda yok (headless yeniden başlatma; keyring henüz planlanan), 443/HTTPS engelli | Kalıcı keyring saklama gelene dek headless restart `HUMMY_BRIDGE_TOKEN` gerektirir; güvenlik duvarında **443 giden** HTTPS'i aç |
...
- Türkçe **tanıtma ve kullanma kılavuzu**, güvenlik uyarıları, üretici/ithalatçı bilgisi, CE işareti, WEEE simgesi, model (`BOX-LITE-01` / `BOX-PRO-01`) ve katalog SKU'su (`hummybox-lite` / `hummybox-pro`) ile seri no ürün ve/veya kutu üzerinde bulunmalı **(resmi kaynaktan teyit edilmeli)**.
...
- **Ağ güvenliği:** köprü **WAN portu açmaz, yerel sunucu/broker dinlemez**; tüm bulut trafiği giden HTTPS'tir (rustls + webpki-roots; kalıcı WSS push planlanan). LAN yazıcı trafiği raw-TCP 9100'dür → köprü ve yazıcıları ayrı VLAN/segment'te tutmak önerilir.
