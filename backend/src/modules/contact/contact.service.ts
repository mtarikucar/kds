...
export class ContactService {
  ...
  async create(dto: CreateContactDto) {
    ...
    if (dto.website && dto.website.trim().length > 0) {
      this.logger.warn(
        `Honeypot triggered from contact form (${maskEmail(dto.email)})`,
      );
      return {
        success: true,
        message:
    ...
    this.logger.log(
      `New contact message ${contactMessage.id} from ${maskEmail(dto.email)}`,
    );
    ...
    if (!adminSent) {
      this.logger.warn(
        `Admin notification not sent for contact ${contactMessage.id}`,
      );
    }
