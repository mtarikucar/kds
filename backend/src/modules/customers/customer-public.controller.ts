...
export class CustomerPublicController {
  ...
  async getLoyaltyBalance(@Query("sessionId") sessionId: string) {
    ...
    const balance = await this.loyaltyService.getBalance(
      session.customerId,
      session.tenantId,
    );
  ...
  async getTierStatus(@Query("sessionId") sessionId: string) {
    ...
    return this.loyaltyService.getTierStatus(
      session.customerId,
      session.tenantId,
    );
  }
  ...
  async verifyOTP(@Body() dto: VerifyOTPDto) {
    ...
    if (result.verified) {
      ...
      if (customer) {
        await this.customersService.markPhoneVerified(
          customer.id,
          session.tenantId,
        );
      }
  ...
  @Throttle({ default: { limit: 10, ttl: 60_000 } })
  @ApiOperation({
    summary: "Generate referral code for the current session customer",
  })
