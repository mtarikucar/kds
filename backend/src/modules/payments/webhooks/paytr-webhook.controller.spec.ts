...
function makeConfig(
  creds: { key?: string | undefined; salt?: string | undefined } = {},
) {
...
describe("PaytrWebhookController", () => {
  ...
  it("dispatches an SP failure with the failure message", async () => {
    ...
    await ctrl.handle(
      signedBody({
        merchant_oid: "SP123",
        status: "failed",
        failed_reason_msg: "declined",
      }),
    );
    expect(selfPay.handleWebhookFailure).toHaveBeenCalledWith(
      "SP123",
      "declined",
    );
  });

  it("swallows an SP settlement error but still returns OK", async () => {
    const ctrl = make();
    selfPay.handleWebhookSuccess.mockRejectedValueOnce(new Error("db blew up"));
    const res = await ctrl.handle(
      signedBody({ merchant_oid: "SP123", status: "success" }),
    );
    expect(res).toBe("OK");
  });
  ...
  it("dispatches a CK- success to checkout settlement", async () => {
    ...
    expect(res).toBe("OK");
    expect(checkoutSettlement.handleSuccess).toHaveBeenCalledWith(
      "CK-9",
      "card",
    );
  });
  ...
  it("swallows a CK- settlement error but still returns OK", async () => {
    const ctrl = make();
    checkoutSettlement.handleSuccess.mockRejectedValueOnce(
