...
describe('ProductsController', () => {
  ...
  it('findAll forwards categoryId + mapped limit/offset', () => {
    ctrl.findAll(req as any, 'cat-1', { limit: 20, offset: 0 } as any);
    expect(svc.findAll).toHaveBeenCalledWith('t1', 'cat-1', { limit: 20, offset: 0 });
  });

  it('findAll passes undefined categoryId/options when omitted', () => {
    ctrl.findAll(req as any);
    expect(svc.findAll).toHaveBeenCalledWith('t1', undefined, {
      limit: undefined,
      offset: undefined,
    });
  });
  ...
  it('reorderImages forwards id, imageIds (from @Body), tenantId', () => {
    ...
    expect(svc.reorderProductImages).toHaveBeenCalledWith('p1', ['i2', 'i1'], 't1');
  });

  it('removeImage forwards both path params + tenantId', () => {
    ctrl.removeImage('p1', 'img-9', req as any);
    expect(svc.removeImageFromProduct).toHaveBeenCalledWith('p1', 'img-9', 't1');
  });
});
