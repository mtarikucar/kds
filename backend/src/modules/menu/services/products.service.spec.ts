...
describe("ProductsService.updateStock", () => {
  ...
  it("throws InsufficientStock when the gate misses but the row exists", async () => {
    ...
    await expect(svc.updateStock("p-1", -50, "t-1")).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("throws NotFound when the row was deleted under us", async () => {
    ...
    await expect(svc.updateStock("p-1", 5, "t-1")).rejects.toThrow(
      NotFoundException,
    );
  });
...
describe("ProductsService.findAll — pagination", () => {
  ...
  it("applies the isAvailable filter when provided (hides soft-deleted/out-of-stock)", async () => {
    await svc.findAll("t-1", undefined, undefined, true);
    const call = prisma.product.findMany.mock.calls[0][0] as any;
    expect(call.where).toEqual({ tenantId: "t-1", isAvailable: true });
  });

  it("does NOT constrain availability when the filter is omitted (full list)", async () => {
    await svc.findAll("t-1");
    const call = prisma.product.findMany.mock.calls[0][0] as any;
    expect(call.where).toEqual({ tenantId: "t-1" });
    expect("isAvailable" in call.where).toBe(false);
  });
});
