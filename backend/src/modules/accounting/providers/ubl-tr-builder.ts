...
import { AccountingInvoiceData } from "../adapters/accounting-adapter.interface";

/**
 * Shared UBL-TR 1.2 invoice XML builder. Extracted verbatim from
 * ForibaEfaturaAdapter so every e-belge integrator adapter (Foriba, Nilvera,
 * …) dispatches the SAME GİB-validated document: Decimal-precise totals
 * (kuruş-drift-free), EFATURA→TICARIFATURA / EARSIVFATURA profile selection,
 * withholding (tevkifat) support and seller/customer party blocks.
 */
export function generateUblTrXml(invoice: AccountingInvoiceData): string {
  ...
  <cac:TaxSubtotal>
    ...
    <cac:TaxCategory>
      <cac:TaxScheme>
        ...
        <cbc:TaxTypeCode>${escapeXml(invoice.withholdingCode || "")}</cbc:TaxTypeCode>
      </cac:TaxScheme>
    </cac:TaxCategory>
  </cac:TaxSubtotal>
  </cac:WithholdingTaxTotal>`
...
${supplierPartyXml(invoice)}
${customerPartyXml(invoice)}
  <cac:TaxTotal>
 ...
 */
function customerPartyXml(invoice: AccountingInvoiceData): string {
  ...
  const taxSchemeXml = taxId
    ...
    <cac:PartyTaxScheme>
      ...
      <cac:TaxScheme><cbc:Name>${escapeXml(taxOffice || "")}</cbc:Name></cac:TaxScheme>
    </cac:PartyTaxScheme>`
    : "";
  return `  <cac:AccountingCustomerParty>
  <cac:Party>
    <cac:PartyName><cbc:Name>${escapeXml(name)}</cbc:Name></cac:PartyName>${taxSchemeXml}
  </cac:Party>
 ...
 */
function supplierPartyXml(invoice: AccountingInvoiceData): string {
  ...
  const taxSchemeXml = vkn
    ? `
    <cac:PartyTaxScheme>
      <cbc:CompanyID>${escapeXml(vkn)}</cbc:CompanyID>
      <cac:TaxScheme><cbc:Name>${escapeXml(taxOffice || "")}</cbc:Name></cac:TaxScheme>
    </cac:PartyTaxScheme>`
    : "";
  const addressXml = address
    ? `
    <cac:PostalAddress><cbc:StreetName>${escapeXml(address)}</cbc:StreetName></cac:PostalAddress>`
    : "";
  const contactXml =
    phone || email
      ? `
  ...
  <cac:Party>
    <cac:PartyName><cbc:Name>${escapeXml(name || "Satici")}</cbc:Name></cac:PartyName>${taxSchemeXml}${addressXml}${contactXml}
  </cac:Party>
