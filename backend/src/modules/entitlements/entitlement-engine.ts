import { classify, EMPTY_ENTITLEMENT_SET, EntitlementGrant, EntitlementSet } from './entitlement.types';
...
interface ReplaceWrapper<T> { __replace: T }

function isReplace<T>(v: unknown): v is ReplaceWrapper<T> {
  return typeof v === 'object' && v !== null && '__replace' in (v as object);
}

export function fold(grants: ReadonlyArray<EntitlementGrant>, now: Date = new Date()): EntitlementSet {
  if (grants.length === 0) return { ...EMPTY_ENTITLEMENT_SET, computedAt: now.toISOString() };
  ...
  for (const g of grants) {
    ...
    if (kind === 'feature') {
      ...
      if (isReplace<string[]>(g.value)) {
        integrationReplacements[g.key] = Array.isArray(g.value.__replace) ? [...g.value.__replace] : [];
        continue;
      }
  ...
  for (const [k, v] of Object.entries(integrationReplacements)) integrations[k] = new Set(v);

  const integrationsOut: Record<string, string[]> = {};
  for (const [k, set] of Object.entries(integrations)) integrationsOut[k] = [...set].sort();
...
export function limitOf(set: EntitlementSet, key: string, fallback = 0): number {
...
export function allowsIntegration(set: EntitlementSet, key: string, providerId: string): boolean {
