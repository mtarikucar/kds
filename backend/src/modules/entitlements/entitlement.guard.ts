...
export class EntitlementGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    for (const r of reqs) {
      if (typeof r === "string") {
        if (!hasFeature(set, r))
          throw new ForbiddenException(`Feature not enabled: ${r}`);
        continue;
      }
      if ("feature" in r) {
        ...
        const usage =
          typeof r.usage === "function" ? await r.usage(req) : r.usage;
        const cap = limitOf(set, r.limit, 0);
        if (cap > 0 && usage >= cap) {
          throw new ForbiddenException(
            `Limit reached for ${r.limit} (${usage}/${cap})`,
          );
        }
      } else if ("integration" in r) {
        if (!allowsIntegration(set, r.integration, r.provider)) {
          throw new ForbiddenException(
            `Integration not enabled: ${r.provider}`,
          );
        }
      }
    }
