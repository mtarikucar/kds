...
export class TableAnalyticsService {
  ...
  async getTableUtilization(
    ...
    const revenueByTable = new Map<
      string,
      { revenue: number; peakHours: Record<number, number> }
    >();
    for (const record of revenueData) {
      const existing = revenueByTable.get(record.tableId) || {
        revenue: 0,
        peakHours: {},
      };
      ...
      if (recordPeakHours) {
        for (const [hour, value] of Object.entries(recordPeakHours)) {
          existing.peakHours[Number(hour)] =
            (existing.peakHours[Number(hour)] || 0) + value;
        }
      }
    ...
    const tableUtilizations: TableUtilizationDto[] = analytics.map((a) => {
      ...
      return {
        ...
        avgOrderValue: a._avg.avgOrderValue
          ? Number(a._avg.avgOrderValue)
          : undefined,
  ...
  async getUtilizationTrends(
    ...
    for (const record of revenueData) {
      ...
      revenueByDate.set(
        dateKey,
        (revenueByDate.get(dateKey) || 0) + Number(record.revenueGenerated),
      );
    }
  ...
  async getCustomerBehavior(
    ...
    for (const record of peakHoursData) {
      ...
      if (peakHours) {
        for (const [hour, value] of Object.entries(peakHours)) {
          arrivalDistribution[Number(hour)] =
            (arrivalDistribution[Number(hour)] || 0) + value;
        }
      }
    }
    ...
    peakDepartureHour = Math.min(
      23,
      peakArrivalHour + Math.ceil((avgDiningTime + avgIdleTime) / 60),
    );
  ...
  private calculateSummary(
    ...
    const avgUtilization =
      tables.reduce((sum, t) => sum + t.utilizationScore, 0) / tables.length;
    ...
    for (const table of tables) {
      if (table.peakHours) {
        for (const [hour, value] of Object.entries(table.peakHours)) {
          hourlyTotals[Number(hour)] =
            (hourlyTotals[Number(hour)] || 0) + value;
        }
      }
    }
    ...
    peakOccupancy =
      maxPossibleOccupancy > 0
        ? (peakOccupancy / maxPossibleOccupancy) * 100
        : 0;
  ...
  private async calculateTableComparisons(
    ...
    for (const r of currentRevenue) {
      currentRevenueMap.set(
        r.tableId,
        (currentRevenueMap.get(r.tableId) || 0) + Number(r.revenueGenerated),
      );
    }
    ...
    for (const r of previousRevenue) {
      previousRevenueMap.set(
        r.tableId,
        (previousRevenueMap.get(r.tableId) || 0) + Number(r.revenueGenerated),
      );
    }

    const previousMap = new Map(
      previousData.map((p) => [p.tableId, p._avg.utilizationScore || 0]),
    );
