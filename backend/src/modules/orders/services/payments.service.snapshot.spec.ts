...
describe('PaymentsService — receipt snapshot persistence', () => {
  ...
  beforeEach(async () => {
    ...
    // PaymentsService.create now calls acquireOrderLock first
    // (SELECT … FOR UPDATE via $queryRaw) — the mock must return at
    // least one row or the helper throws NotFound and never reaches
    // the snapshot path. Added as part of the Faz 1 row-lock refactor.
    (prisma.$queryRaw as any).mockResolvedValue([{ id: baseOrder.id }]);

    // payByItems / new branches also consult pending self-pay intents
    // for the reservation check — stub empty so the snapshot spec
