...
export class PaymentsController {
  ...
  @ApiOperation({
    summary: "Create a payment for an order (ADMIN, MANAGER, WAITER)",
  })
  @ApiResponse({ status: 201, description: "Payment successfully created" })
  @ApiResponse({
    status: 400,
    description: "Invalid data or order already paid",
  })
  ...
  @Roles(UserRole.ADMIN, UserRole.MANAGER, UserRole.WAITER)
  @ApiOperation({
    summary: "Get all payments for an order (ADMIN, MANAGER, WAITER)",
  })
  ...
  @ApiResponse({
    status: 200,
    description: "Order paid/remaining items summary",
  })
  @ApiResponse({ status: 404, description: "Order not found" })
  ...
  @ApiResponse({
    status: 201,
    description: "Order written off, table released",
  })
  @ApiResponse({
    status: 400,
    description:
      "Order is already paid, cancelled, or has nothing left to write off",
  })
  @ApiResponse({ status: 403, description: "Insufficient permissions" })
  writeOff(
    @Param("orderId") orderId: string,
