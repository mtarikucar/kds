...
export class StockController {
  ...
  // Catalog-level alert: Product is tenant-scoped (per-tenant menu,
  // no branchId column), so this view is intentionally tenant-wide.
  // SkipBranchScope satisfies the lint rule that every handler must
  // declare its scope axis explicitly.
  @Get("alerts")
  @Roles(UserRole.ADMIN, UserRole.MANAGER)
  @SkipBranchScope()
  ...
  getLowStockAlerts(@Request() req, @Query("threshold") threshold?: string) {
    ...
    if (threshold !== undefined) {
      ...
      if (!Number.isFinite(parsed) || parsed < 0 || parsed > 1_000_000) {
        throw new BadRequestException(
          "threshold must be a non-negative integer ≤ 1,000,000",
        );
      }
      thresholdNum = parsed;
    }
