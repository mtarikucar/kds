...
export class SuperadminInstallationController {
  ...
  @ApiOperation({
    summary:
      "Ops queue across all tenants (status / assignedTo filter optional)",
  })
  list(
  ...
  schedule(@Param("id") id: string, @Body() body: ScheduleInstallationDto) {
    ...
    return this.installation.scheduleByOps(
      id,
  ...
  @Patch(":id/complete")
  @ApiOperation({
    summary: "Mark installation done with optional close-out note",
  })
...
export class WarrantyController {
  ...
  @Post(":id/claims")
  @ApiOperation({
    summary: "File a warranty claim against a serial (ADMIN/MANAGER)",
  })
  file(
    @Req() req: any,
    @Param("id") id: string,
...
export class SuperadminShipmentsController {
  ...
  @Post(":orderId")
  create(@Param("orderId") orderId: string, @Body() body: CreateShipmentDto) {
