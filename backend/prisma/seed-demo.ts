...
const PRODUCT_IMAGES: Record<string, string> = {
  ...
  Menemen:
    "https://images.unsplash.com/photo-1623689048105-a17b1e1936b8?w=800&q=80",
  "Sucuklu Yumurta":
  ...
  "Sigara Boregi":
    "https://images.unsplash.com/photo-1548340748-6d2b7d7da280?w=800&q=80",
  "Yaprak Sarma":
  ...
  "Ali Nazik":
    "https://images.unsplash.com/photo-1606491048802-8342506d6471?w=800&q=80",
  Kunefe:
...
async function main() {
  ...
  const tenant = await prisma.tenant.create({
    data: {
      ...
      reportEmails: [
        "rapor@sultanahmet-sofra.com",
        "ahmet@sultanahmet-sofra.com",
      ],
  ...
  const categoryData = [
    {
      name: "Kahvalti",
      description: "Geleneksel Turk kahvaltisi",
      displayOrder: 1,
    },
    ...
    {
      name: "Izgara",
      description: "Mangal ve izgara cesitleri",
      displayOrder: 5,
    },
    {
      name: "Ana Yemekler",
  ...
  const productDefs: {
    ...
    // Kahvalti
    {
      ...
      price: 450,
      desc: "2 kisilik zengin kahvalti tabagi",
    },
    {
      cat: "Kahvalti",
      name: "Menemen",
      price: 120,
      desc: "Domatesli biberli yumurta",
    },
    {
      cat: "Kahvalti",
      name: "Sucuklu Yumurta",
      price: 130,
      desc: "Kasap sucugu ile sahanda yumurta",
    },
    // Mezeler
    { cat: "Mezeler", name: "Humus", price: 90, desc: "Tahinli nohut ezme" },
    {
      ...
      name: "Babaganus",
      price: 85,
      desc: "Kozlenmis patlican salatasi",
    },
    {
      cat: "Mezeler",
      name: "Sigara Boregi",
      price: 95,
      desc: "El acmasi sigara boregi",
    },
    {
      cat: "Mezeler",
      name: "Yaprak Sarma",
      price: 80,
      desc: "Zeytinyagli yaprak sarma",
    },
    {
      cat: "Mezeler",
      name: "Acili Ezme",
      price: 75,
      desc: "Taze acili domates ezme",
    },
    // Salatalar
    {
      cat: "Salatalar",
      name: "Coban Salata",
      price: 70,
      desc: "Domates, salatalik, sogan, maydanoz",
    },
    {
      cat: "Salatalar",
      name: "Sezar Salata",
      price: 110,
      desc: "Tavuklu sezar salata",
    },
    // Pideler
    {
      cat: "Pideler",
    ...
    { cat: "Pideler", name: "Kasarli Pide", price: 140, desc: "Kasarli pide" },
    {
      ...
      name: "Karisik Pide",
      price: 180,
      desc: "Karisik malzemeli pide",
    },
    {
      cat: "Pideler",
      name: "Kusbasi Pide",
      price: 190,
      desc: "Kusbasi etli pide",
    },
    // Izgara
    {
      cat: "Izgara",
      name: "Adana Kebap",
      price: 280,
      desc: "Acili el kiymasi kebap",
    },
    {
      cat: "Izgara",
      name: "Urfa Kebap",
      price: 280,
      desc: "Acisiz el kiymasi kebap",
    },
    {
      cat: "Izgara",
      name: "Kuzu Pirzola",
      price: 420,
      desc: "4 parca kuzu pirzola",
    },
    {
      cat: "Izgara",
      name: "Karisik Izgara",
      price: 380,
      desc: "Adana, tavuk, kofte, pirzola",
    },
    {
      cat: "Izgara",
      name: "Tavuk Sis",
      price: 220,
      desc: "Marine edilmis tavuk sis",
    },
    ...
    // Ana Yemekler
    {
      ...
      name: "Iskender",
      price: 300,
      desc: "Bursa iskender kebap",
    },
    {
      cat: "Ana Yemekler",
      name: "Hunkar Begendi",
      price: 290,
      desc: "Patlican puresi ustunde kusbasi",
    },
    {
      cat: "Ana Yemekler",
      name: "Etli Guvec",
      price: 260,
      desc: "Sebzeli etli guvec",
    },
    {
      cat: "Ana Yemekler",
      name: "Ali Nazik",
      price: 310,
      desc: "Yogurtlu patlican ustunde kebap",
    },
    // Tatlilar
    {
      cat: "Tatlilar",
      name: "Kunefe",
      price: 180,
      desc: "Antep fistigli kunefe",
    },
    {
      cat: "Tatlilar",
      name: "Baklava",
      price: 160,
      desc: "Fistikli baklava (6 dilim)",
    },
  ...
  console.log(
    `✅ Product images: ${imgSuccess} downloaded, ${imgFail} skipped`,
  );
  ...
  const etPisirme = await prisma.modifierGroup.create({
    data: {
      name: "et_pisirme",
      displayName: "Et Pisirme Derecesi",
      selectionType: "SINGLE",
      isRequired: false,
      minSelections: 0,
      maxSelections: 1,
  ...
  pmgData.push({
    productId: products["Turk Kahvesi"].id,
    groupId: kahveCesidi.id,
  });
  ...
  async function createOrder(opts: {
    ...
    const createdAt = new Date(
      now.getTime() - (opts.createdMinutesAgo || 0) * 60000,
    );

    let totalAmount = 0;
    const itemsData = opts.items.map((it) => {
      const prod = products[it.productName];
      const modTotal = (it.modifiers || []).reduce(
        (sum, m) => sum + m.price,
        0,
      );
    ...
    for (const it of itemsData) {
      const orderItem = await prisma.orderItem.create({
        data: {
          ...
          status:
            opts.status === "PENDING" || opts.status === "PENDING_APPROVAL"
              ? "PENDING"
  ...
  const modBuyukIcecek = icecekBoyutu.modifiers.find(
    (m) => m.name === "buyuk_boy",
  )!;
  ...
  await createOrder({
    ...
    status: "PAID",
    tableId: tables["3"].id,
    userId: users.mehmet.id,
    customerId: customers["Ayse Yildiz"].id,
    ...
    items: [
      {
        ...
        qty: 2,
        modifiers: [
          { modifierId: modOrta.id, price: 0 },
          { modifierId: modAci.id, price: 5 },
        ],
      },
  ...
  await createOrder({
    ...
    tableId: tables["5"].id,
    userId: users.mehmet.id,
    customerId: customers["Hasan Korkmaz"].id,
  ...
  await createOrder({
    type: "TAKEAWAY",
    status: "PAID",
    userId: users.zeynep.id,
    customerName: "Misafir",
    items: [
      {
        productName: "Karisik Pide",
        qty: 2,
        modifiers: [{ modifierId: modBuyukPide.id, price: 30 }],
      },
    ...
    items: [
      {
        ...
        qty: 3,
        modifiers: [{ modifierId: modSade.id, price: 0 }],
      },
  ...
  await createOrder({
    ...
    tableId: tables["6"].id,
    userId: users.mehmet.id,
    customerId: customers["Kemal Taskin"].id,
    customerName: "Kemal Taskin",
    items: [
      {
        productName: "Karisik Izgara",
        qty: 1,
        modifiers: [{ modifierId: modOrta.id, price: 0 }],
      },
      ...
      { productName: "Acili Ezme", qty: 1 },
      {
        productName: "Salgam",
        qty: 2,
        modifiers: [{ modifierId: modBuyukIcecek.id, price: 20 }],
      },
    ],
    paid: true,
    paymentMethod: "CARD",
  ...
  await createOrder({
    ...
    tableId: tables["1"].id,
    userId: users.mehmet.id,
    customerId: customers["Fatma Aksoy"].id,
    customerName: "Fatma Aksoy",
    items: [
  ...
  await createOrder({
    ...
    userId: users.zeynep.id,
    customerId: customers["Selin Dogan"].id,
    customerName: "Selin Dogan",
    items: [
  ...
  await createOrder({
    ...
    tableId: tables["2"].id,
    userId: users.mehmet.id,
    customerId: customers["Emre Sahin"].id,
    customerName: "Emre Sahin",
    items: [
      {
    ...
    items: [
      {
        productName: "Adana Kebap",
        qty: 3,
        modifiers: [{ modifierId: modAci.id, price: 5 }],
      },
  ...
  await createOrder({
    ...
    items: [{ productName: "Kusbasi Pide", qty: 1 }],
  ...
  await createOrder({
    ...
    userId: users.zeynep.id,
    source: "GETIR",
    externalOrderId: "GTR-9988771",
    customerName: "Getir Musteri",
    notes: "Musteri iptal",
    items: [{ productName: "Iskender", qty: 1 }],
  ...
  const stockMovements = [
    ...
    {
      product: "Kuzu Pirzola",
      type: "IN",
      qty: 20,
      reason: "Kasap teslimat",
      user: "mustafa",
    },
    {
      product: "Serpme Kahvalti",
      type: "IN",
      qty: 15,
      reason: "Kahvalti malzemesi",
      user: "mustafa",
    },
    {
      product: "Kunefe",
    ...
    },
    {
      ...
      type: "OUT",
      qty: 3,
      reason: "Son kullanma tarihi gecmis",
      user: "mustafa",
    },
    {
      product: "Ayran",
  ...
  await prisma.reservationSettings.create({
    data: {
      ...
      customMessage:
        "Rezervasyonunuz onaylandiktan sonra size bilgi verilecektir.",
    },
  });
  ...
  const notificationData = [
    {
      ...
      message: "Kuzu Pirzola stogu 5 adetten az. Lutfen siparis verin.",
      type: "STOCK",
      priority: "HIGH",
    },
    {
      ...
      message:
        "Hasan Korkmaz 8 kisilik yarin aksam 19:30 icin rezervasyon talebi olusturdu.",
      type: "INFO",
      priority: "NORMAL",
    },
    {
      ...
      message:
        "Yeni Yemeksepeti siparisi alindi: Karisik Izgara, Coban Salata, 2x Ayran.",
      type: "ORDER",
      priority: "HIGH",
    },
    {
      title: "Gunluk Rapor",
      message:
        "Dunku toplam ciro: 12.450 TL | Toplam siparis: 47 | Ortalama siparis: 264.89 TL",
      type: "INFO",
      priority: "LOW",
    },
    {
      title: "QR Menu Siparisi Bekliyor",
      message: "Masa 2'den yeni QR menu siparisi onay bekliyor.",
      type: "ORDER",
      priority: "NORMAL",
    },
  ];
