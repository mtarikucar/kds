...
**Gereksinim:** `personnelManagement` — **Personel Yönetimi** modülü (990 ₺/yıl, lisans ön koşuluyla)
...
<Callout type="info">
  Personel, **Personel Yönetimi** modülüyle (990 ₺/yıl; Bakım, Destek ve Güncelleme 4.900 ₺/yıl
  ön koşuldur) açılır. Modül sizde yoksa menüde Personel görünmez ve doğrudan URL
  satın alma kartı gösterir.
...
| **Kartlı Vardiya (Card Shift)** | RFID kart ataması — personel başına tek satır, yalnız son 4 hane |
...
## Kart ile giriş-çıkış yapma

`personnelManagement`'a **ek olarak** **Kartlı Vardiya** modülünü
(`cardShift`, ₺4.000 tek seferlik, lisans ön koşuluyla) gerektirir.

1. **Kartları atayın.** Ekip → Kartlı Vardiya → "Kart ata", ardından kartı
   odaklanan alana okutun. Ekranda yalnız son dört hane görünür.
2. **İstasyon ekranını açın.** Sol menü → Kartlı Vardiya (`/card-shift`),
   okuyucunun takılı olduğu tablet veya PC'de. Ekranı açık bırakın.
3. **Okutma.** Günün ilk okutması giriş, ikincisi çıkış damgalar; moladayken
   okutma molayı bitirir. Mola başlatmak uygulama içinden yapılır.
4. **Tekrarlanan okutmalar.** 10 saniye içindeki ikinci okutma yok sayılır —
   bazı okuyucular tek okutmada iki kez yazar.
5. **Tanınmayan kartlar.** Kimseye ait olmayan (veya pasifleştirilmiş bir
   personele ait) kart "Kart tanınmadı" gösterir. Numara ekranda hiçbir yerde
   gösterilmez ve log'a yazılmaz.

Bir kartı iptal etmek geçmişteki tüm puantaj kayıtlarını korur — kartla
damgalandığı bilgisi dahil.

## İlgili ekranlar

- **Kullanıcılar** (`/admin/users`) — sistem hesabı, rol ve durum yönetimi (ücretsiz çekirdeğe dahildir).
