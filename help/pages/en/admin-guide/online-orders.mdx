...
**Requires:** `deliveryIntegration` — at least one delivery-platform integration (**Yemeksepeti**, **Getir** or **Trendyol Yemek**; ₺2,490/year each, with the licence as a prerequisite)
...
<Callout type="info">
  Each platform is configured on its own card. The catalog sells the **Yemeksepeti**,
  **Getir** and **Trendyol Yemek** integrations separately (₺2,490/year each, with the
  licence as a prerequisite). Until you open a card, enter credentials and turn the
  integration on, **no orders are received** from that platform.
</Callout>
