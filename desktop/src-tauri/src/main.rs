...
// The binary IS the Tauri desktop app, so its whole body is gated behind the
// default `app` feature (which pulls in `tauri`). The unit tests live in the
// `lib` target and run via `cargo test --lib --no-default-features`, which
// never compiles this file — that is how the hardware logic is tested without
// Tauri/webkit. Under `--no-default-features` the binary collapses to the
// no-op `main` stub below; under default features it is the full app exactly
// as before (behavior-preserving).
#![cfg_attr(not(feature = "app"), allow(unused))]

#[cfg(not(feature = "app"))]
fn main() {
    // No-op: the desktop app requires the `app` feature. Built without it
    // (hardware-logic-only mode) there is nothing to run.
}

// Hardware modules (binary's own copy — the `lib` target has an independent
// copy for the unit tests). Gated with the binary so a `--no-default-features`
// build pulls in none of the Tauri-bound code.
#[cfg(feature = "app")]
mod bluetooth;
#[cfg(feature = "app")]
mod escpos;
...
/// Real desktop-app entry point under the default `app` feature.
#[cfg(feature = "app")]
fn main() {
    app::run();
}
...
#[cfg(feature = "app")]
mod app {
    //! The entire desktop application. Lives in a feature-gated module so a
    //! single `#[cfg(feature = "app")]` gates the whole Tauri surface (command
    //! handlers, state, `generate_context!`) in one place, instead of scattering
    //! the attribute across ~40 items.
    use crate::bluetooth::{BluetoothManager, PrinterCommand, ScannedDevice};
    use crate::hardware::config::DeviceConfig;
    use crate::hardware::status::DeviceStatus;
    use crate::hardware::{self, HardwareManager};
    use std::sync::Arc;
    use tauri::State;
    ...
    async fn init_bluetooth(state: State<'_, AppState>) -> Result<String, String> {
        let manager = BluetoothManager::new().await.map_err(|e| e.to_string())?;
    ...
    async fn connect_device(
        ...
        let bt = state.bluetooth.lock().await;
        let manager = bt.as_ref().ok_or("Bluetooth not initialized")?;
    ...
    async fn initialize_hardware(
        ...
        if has_bluetooth {
            let mut bt = state.bluetooth.lock().await;
            if bt.is_none() {
                *bt = Some(BluetoothManager::new().await.map_err(|e| e.to_string())?);
            }
        }
    ...
    #[tauri::command]
    async fn test_device(device_id: String, state: State<'_, AppState>) -> Result<String, String> {
        let bt = state.bluetooth.lock().await;
        let manager = bt
            .as_ref()
    ...
    async fn list_printers(state: State<'_, AppState>) -> Result<Vec<LegacyPrinterInfo>, String> {
        ...
        let printers = guard
            ...
            .devices
            .iter()
            .filter(|d| matches!(d.device_type, hardware::config::DeviceType::ThermalPrinter))
