...
// Plans and subscriptions are gone (v3.3.0). Nothing writes to that rail, so
// hooks for it could only ever return empty lists and mutate rows the
// entitlement engine no longer reads. The catalog lives in
// superadminMarketplaceApi; a tenant's licence state and comps live on
// /v1/superadmin/marketplace.
