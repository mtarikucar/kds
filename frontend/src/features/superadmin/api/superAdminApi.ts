...
export interface SuperAdminMarketerListItem {
  id: string;
  email: string;
  firstName: string;
  lastName: string;
...
export interface SuperAdminCommissionListItem {
  id: string;
  ...
  lead?: { id: string; businessName: string; source: string } | null;
}
...
export const useSuperAdminMarketers = (filters: SuperAdminMarketerFilter = {}) => {
  return useQuery({
    queryKey: ['superadmin', 'marketers', filters],
    queryFn: async (): Promise<PaginatedResponse<SuperAdminMarketerListItem>> => {
      const response = await superAdminApi.get('/superadmin/marketers', { params: filters });
      return response.data;
    },
  });
};

export const useSuperAdminMarketer = (id: string | null) => {
  return useQuery({
    queryKey: ['superadmin', 'marketers', id],
    queryFn: async () => {
      const response = await superAdminApi.get(`/superadmin/marketers/${id}`);
      return response.data;
    },
    enabled: !!id,
  });
...
export const useUpdateMarketerStatus = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, status }: { id: string; status: 'ACTIVE' | 'INACTIVE' }) => {
      const response = await superAdminApi.patch(`/superadmin/marketers/${id}/status`, {
        status,
      });
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['superadmin', 'marketers'] });
    },
  });
};

export const useRegenerateMarketerReferralCode = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (id: string) => {
      const response = await superAdminApi.post(
        `/superadmin/marketers/${id}/regenerate-referral-code`,
      );
      return response.data;
    },
    onSuccess: () => {
...
export const useSuperAdminCommissions = (filters: SuperAdminCommissionFilter = {}) => {
  return useQuery({
    queryKey: ['superadmin', 'commissions', filters],
    queryFn: async (): Promise<
      PaginatedResponse<SuperAdminCommissionListItem> & {
        summary: { totalAmount: number | string; totalCount: number };
      }
    > => {
      const response = await superAdminApi.get('/superadmin/commissions', { params: filters });
      return response.data;
    },
  });
};
...
export const useBulkApproveCommissions = () => {
  ...
  return useMutation({
    mutationFn: async (ids: string[]) => {
      const response = await superAdminApi.post('/superadmin/commissions/bulk-approve', {
        ids,
      });
      return response.data;
    },
...
export const useExportCommissions = () => {
  return useMutation({
    mutationFn: async (filters: SuperAdminCommissionFilter) => {
      const response = await superAdminApi.get('/superadmin/commissions/export.csv', {
        params: filters,
        responseType: 'blob',
      });
      return response.data;
