import { X, Check, User, MapPin, Clock, MessageSquare } from 'lucide-react';
  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-0 bottom-0 w-full md:w-[500px] bg-white shadow-2xl z-50 overflow-hidden flex flex-col">
        {/* Header */}
        <div className="bg-gradient-to-r from-blue-500 to-indigo-500 text-white px-5 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-white/20 rounded-lg">
              <User className="h-5 w-5" />
            </div>
            <div>
              <h2 className="text-lg font-heading font-bold">{t('waiterRequests.title')}</h2>
              <p className="text-sm text-white/80">{waiterRequests.length} {waiterRequests.length !== 1 ? t('waiterRequests.activeRequests') : t('waiterRequests.activeRequest')}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-4 bg-slate-50/50">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-64 text-slate-400">
              ...
              <p className="text-lg font-medium text-slate-500">{t('waiterRequests.noRequests')}</p>
              <p className="text-sm text-slate-400">{t('waiterRequests.allHandled')}</p>
            </div>
          ) : (
                  ...
                  <div className="bg-slate-50/80 px-5 py-4 border-t border-slate-100 flex gap-3">
                    {request.status === 'PENDING' ? (
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-blue-500 text-white rounded-xl hover:bg-blue-600 font-semibold transition-all duration-200 shadow-sm hover:shadow disabled:opacity-50 disabled:cursor-not-allowed"
                      >
...
export default WaiterRequestsPanel;
