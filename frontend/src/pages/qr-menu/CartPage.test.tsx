...
vi.mock('react-router-dom', async () => {
  ...
  return {
    ...
    useSearchParams: () => [{ get: (k: string) => (k === 'tableId' ? tableIdParam : null) }],
  };
});
...
vi.mock('../../components/qr-menu/TableSelectionModal', () => ({
  default: ({ isOpen }: any) => (isOpen ? <div data-testid="table-modal" /> : null),
}));
...
describe('CartPage — order submission', () => {
  it('POSTs the mapped cart, clears it and navigates to orders on success', async () => {
    ...
    expect(navigate).toHaveBeenCalledWith(expect.stringContaining('/qr-menu/t-1/orders'));
  });

  it('toasts the server error and does NOT clear the cart on failure', async () => {
    post.mockRejectedValue({ response: { data: { message: 'kitchen closed' } } });
    await loadAndSubmit();

    await waitFor(() => expect(toastError).toHaveBeenCalledWith('kitchen closed'));
