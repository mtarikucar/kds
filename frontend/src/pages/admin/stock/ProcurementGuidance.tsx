...
// Procurement guidance. Until v3.6.7 these two sections were a "Tedarik
// Rehberi" tab of their own; they now bracket the suppliers hub — the buy list
// opens it (it is the reason you came), the channel guide closes it (reference
// material, read once at setup). They are separate exports precisely so the
// hub can place them at opposite ends rather than side by side.
//
// Both call useGuidance(); react-query dedupes the shared key, so rendering
// the pair still issues one request.
...
const useWhyLine = () => {
  ...
  return (s: GuidanceSource): string => {
...
/** "Bugün alınması gerekenler" — par-altı malzemeler + tek tık taslak sipariş. */
export function BuyListSection() {
  ...
  // The error state lives here rather than in both sections: one failed query
  // must not stack two identical messages down the page.
...
/** "Kanal rehberi" — kategori × ölçek tedarik önerileri. Referans malzeme. */
export function ChannelGuideSection() {
  const { t } = useTranslation('stock');
  const { data, isError } = useGuidance();
  const [tierView, setTierView] = useState<VolumeTier | null>(null);

  const tier = tierView ?? data?.volumeTier ?? 'SMALL_CAFE';

  // BuyListSection already reports the failure; stay quiet rather than repeat it.
