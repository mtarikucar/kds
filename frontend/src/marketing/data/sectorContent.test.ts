...
describe("sector solution content", () => {
  ...
  it.each(SECTORS)(
    "$slug: hero image + featured module slugs are valid",
    (s) => {
  ...
  it("delivery-heavy sectors lean on platform integration, not own-courier", () => {
    ...
    for (const slug of ["pizza", "fast-food", "bulut-mutfak"]) {
      ...
      expect(blob, `${slug} should mention delivery platforms`).toMatch(
        /yemeksepeti|getir|trendyol|migros/,
      );
      if (blob.includes("gps") || blob.includes("kurye takip")) {
        expect(blob, `${slug} mentions courier/GPS — must disclaim it`).toMatch(
          /sunmaz|sunmuyoruz|bulunmuyor|yoktur|\byok\b|değil/,
        );
      }
    }
  });
