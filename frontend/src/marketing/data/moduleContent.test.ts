...
import { MODULE_CONTENT, CONTENT_META, getModuleCopy, MODULE_SLUGS } from './moduleContent';
...
describe('module deep-dive content', () => {
  it('has generated copy for all 8 modules (not fallback)', () => {
    for (const slug of MODULE_SLUGS) {
      expect(MODULE_CONTENT[slug], `missing content: ${slug}`).toBeDefined();
    }
    expect(Object.keys(MODULE_CONTENT).length).toBe(8);
  });
